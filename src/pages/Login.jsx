...
export default function Login() {
  ...
  const { user, loading: authLoading } = useAuth(); // Global auth state
  const [isProcessing, setIsProcessing] = useState(false); // Local button state
  const MENTOR_EMAIL = "nishant53195@gmail.com";

  // --- PERSISTENCE LOGIC ---
  useEffect(() => {
    // If Firebase already knows the user, redirect them immediately
    if (!authLoading && user) {
      if (user.email === MENTOR_EMAIL) {
        navigate("/mentor-os-dashboard-53195");
      } else {
        // We check their status in the Dashboard component, 
        // or you can add a quick check here if you prefer.
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      // 2. Is it a Student? Check Firestore
  ...
  // Prevent "flicker" - don't show the login page if we are still checking auth status
  if (authLoading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-950">
        <div className="animate-pulse text-blue-500 font-bold text-xl tracking-widest">
          PROGRESSIFY OS IS LOADING...
        </div>
      </div>
    );
  }
